...
{
    public UserFollowRepository(Database database) : base(database)
    {
    }

    public async Task<List<UserFollow>> SearchAsync(
        string userNameOrNovelName,
        PaginationParameters pagination,
        bool isDescending)
    {
        var query = Database.Collection<UserFollow>().AsQueryable();
